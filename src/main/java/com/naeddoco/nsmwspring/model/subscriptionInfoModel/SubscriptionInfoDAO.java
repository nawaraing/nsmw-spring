...
public class SubscriptionInfoDAO {
	...
	private static final String SELECTALL_SUBSCRIPTION_INFO = "SELECT " +
															  ...
															  "LEFT JOIN (SELECT SIP.SUBSCRIPTION_INFO_ID, SUM(SIP.PURCHASE_PRICE * SIP.QUANTITY) AS TOTAL_PRICE FROM SUBSCRIPTION_INFO_PRODUCT SIP GROUP BY SIP.SUBSCRIPTION_INFO_ID) AS TOTAL_PRICE_TABLE ON SI.SUBSCRIPTION_INFO_ID = TOTAL_PRICE_TABLE.SUBSCRIPTION_INFO_ID " +
															  "WHERE MEMBER_ID = ?";
